...
sample_survival <- function(n, hazard){
  # CDF F(t) = 1 - exp(-hazard * t)
  # inverse CDF: F^{-1}(t) = -log(1 - u) / hazard
  u <- runif(n)
  return (-log(1 - u) / hazard)
}
...
data$Y <- ifelse(data$S == 1,
                 sample_survival(n, 2), 
                 ifelse(data$S == 2,
                        sample_survival(n, 1.5 - 1 * data$Z),
                        sample_survival(n, 1))
)

data$C <- rbinom(n, 1, 0.3)
