...
PSFormula <- function(formula, data) {
  ...
  has_random_effect <- !is.null(lme4::findbars(formula))
  fixed_formula <- lme4::nobars(formula)

  LHS <- if (length(formula) == 3) formula[[2]] else NULL
  LHS_names <- extract_symbols(LHS)

  terms_obj <- stats::terms.formula(fixed_formula)
  has_intercept <- as.logical(attr(terms_obj, "intercept"))
  model_matrix <- stats::model.matrix(fixed_formula, data)

  random_eff_list <- if (has_random_effect) {
    parse_random_effects(formula, data)
  }
  ...
  structure(
    list(
      ...
      data = data,
      fixed_eff_formula = fixed_formula,
      response_names = LHS_names,
      ...
      random_eff_list = random_eff_list
    ),
    class = "PSFormula"
  )
}

#' Recursively extract symbol names from an AST node
#' @noRd
extract_symbols <- function(node) {
  if (is.name(node)) return(as.character(node))
  if (is.call(node)) return(unlist(unique(sapply(node[-1], extract_symbols))))
  NULL
}

#' Parse random effect terms using lme4
#' @noRd
parse_random_effects <- function(formula, data) {
  lf <- lme4::lFormula(formula, data)
  mapply(
    function(mat, terms, factors) {
      list(matrix = t(as.matrix(mat)), terms = terms, factors = factors)
    },
    lf$reTrms$Ztlist,
    lf$reTrms$cnms,
    lf$reTrms$flist,
