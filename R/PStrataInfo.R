...
PStrataInfo <- function(strata, ER = NULL) {
  parsed <- parse_strata_input(strata)
  ER_list <- resolve_ER(
    ER, parsed$ER_from_string, parsed$strata_names,
    parsed$canonical_names, length(strata)
  )

  new_PStrataInfo(
    strata_matrix      = parsed$strata_matrix,
    strata_names       = parsed$strata_names,
    ER_list            = ER_list,
    max_postrand_level = parsed$max_postrand_level
  )
}

# --- Internal constructor ---

#' Low-level constructor for PStrataInfo
#' @noRd
new_PStrataInfo <- function(strata_matrix, strata_names, ER_list, max_postrand_level) {
  structure(
    list(
      num_strata         = nrow(strata_matrix),
      num_treatment      = ncol(strata_matrix),
      num_postrand_var   = length(max_postrand_level),
...
#' Parse the strata argument (string vector or list of matrices)
#' @noRd
parse_strata_input <- function(strata) {
  ER_from_string <- NULL
  ...
  if (is.character(strata)) {
    ...
    ER_from_string <- stringr::str_ends(strata, stringr::fixed("*"))
    strata_clean <- stringr::str_remove(strata, stringr::fixed("*"))
    canonical_names <- strata_clean
    strata_names <- ifelse(strata_names == "", strata_clean, strata_names)

    strata_split <- stringr::str_split(strata_clean, stringr::fixed("|"))
    raw_matrices <- lapply(strata_split, function(parts) {
      digit_matrix <- do.call(cbind, stringr::str_split(parts, ""))
      t(apply(digit_matrix, c(1, 2), as.integer))
    })
  } else if (is.list(strata)) {
    raw_matrices <- lapply(strata, function(x) {
      if (!is.matrix(x)) matrix(x, nrow = 1) else x
    })
    strata_names <- names(strata)
    stopifnot(
      "All strata must be named if the matrix initialization is used." =
        !is.null(strata_names) && all(strata_names != "")
    )
  } else {
    stop("strata must be a character vector or a named list of matrices.")
  }

  stacked <- abind::abind(raw_matrices, along = 3)
  max_postrand_level <- apply(stacked, 1, max)
  strata_matrix <- apply(stacked, c(3, 2), function(x) {
    t(c(x, 0)) %*% c(1, cumprod(max_postrand_level + 1))
  })

  list(
    strata_matrix      = strata_matrix,
    strata_names       = strata_names,
    ER_from_string     = ER_from_string,
    canonical_names    = canonical_names,
    max_postrand_level = max_postrand_level
  )
}

#' Resolve the ER specification from strata shorthand and explicit ER argument
#' @noRd
resolve_ER <- function(ER, ER_from_string, strata_names, canonical_names, n_strata) {
  if (is.null(ER)) return(ER_from_string)

  if (!is.null(ER_from_string) && any(ER_from_string)) {
    warning("Shorthand specification of ER is ignored and overwritten, since the ER argument is provided with value.")
  }

  if (is.logical(ER)) return(ER)

  ER_list <- rep(FALSE, n_strata)
  for (word in ER) {
    matched <- FALSE
    if (word %in% strata_names) {
      ER_list[which(strata_names == word)] <- TRUE
      matched <- TRUE
    } else if (!is.null(canonical_names) && word %in% canonical_names) {
      ER_list[which(canonical_names == word)] <- TRUE
      matched <- TRUE
    }
    if (!matched) {
