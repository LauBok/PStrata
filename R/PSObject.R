...
PSObject <- function(
  ...
  # Coerce formulas if needed
  if (!inherits(S.formula, "PSFormula")) S.formula <- PSFormula(S.formula, data)
  if (!inherits(Y.formula, "PSFormula")) Y.formula <- PSFormula(Y.formula, data)
  if (!inherits(strata, "PStrataInfo"))  strata <- PStrataInfo(strata, ER)

  Z_levels <- resolve_treatment_levels(S.formula, strata$num_treatment)
  SZDG <- build_SZDG_table(strata)
...
#' Determine treatment levels from data
#' @noRd
resolve_treatment_levels <- function(S.formula, num_treatment) {
...
#' Build the SZDG (Stratum-Treatment-Confounder-Group) mapping table
#'
#' Each row maps a (Stratum, Treatment arm, observed D value, outcome Group).
#' The Group index determines which outcome model parameters are shared:
#' strata with ER that share the same D value across treatment arms
#' get the same Group index, enforcing shared outcome parameters.
#'
#' @param strata_info A PStrataInfo object
#' @return A matrix with columns S, Z, D, G
#' @noRd
build_SZDG_table <- function(strata_info) {
  SZDG <- matrix(nrow = 0, ncol = 4)
  colnames(SZDG) <- c("S", "Z", "D", "G")
  G_id <- 0

  for (s_idx in seq_len(strata_info$num_strata)) {
    S_id <- s_idx - 1
    for (z_idx in seq_len(strata_info$num_treatment)) {
      Z_id <- z_idx - 1
      D_id <- strata_info$strata_matrix[s_idx, z_idx]

      # Under ER, reuse the Group id if same stratum & D value already seen
      tmp_G_id <- NULL
      if (strata_info$ER_list[s_idx] && nrow(SZDG) > 0) {
        match_rows <- which(SZDG[, "S"] == S_id & SZDG[, "D"] == D_id)
        if (length(match_rows) > 0) {
          tmp_G_id <- SZDG[match_rows[1], "G"]
        }
      }
